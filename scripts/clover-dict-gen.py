...
class DictGenerator:
    ...
    def mergeDict(self, text, weight=1, min_freq=0, info=""):
        """
    ...
    def getWordDictText(self):
        ...
        """
        # 按频率倒序排序
        word_list = [(key[0], key[1], self.word_dict[key])
        ...
        for word_st in word_list:
            text_dict += word_st[0] + '\t' + \
                word_st[1] + '\t' + str(word_st[2]) + '\n'
        return text_dict

    def getParseDictText(self, info=""):
        """
            生成词组的 rime 字典文本
            由于词典量较大，所以需要显示转换进度
        ...
        """
        # 按频率倒序排序
        phrase_list = [(phrase, self.phrase_r[phrase])
        ...
        count = 0
        text_phrase = ''
        for phrase_st in tqdm(phrase_list, info):
            # 通过 pypinyin 库获取到词组的拼音
            phrase_pinyin = map(
                self.fixPinyin, pypinyin.lazy_pinyin(phrase_st[0]))
...
def main(minfreq):
    ...
    submodules_dir = os.path.join(current_dir, '..', 'submodules')
    # 合并 360万 的词库
    chinese_dictionary_dict = os.path.join(
        submodules_dir, 'chinese-dictionary-3.6million', '词典360万（个人整理）.txt')
    text = open(chinese_dictionary_dict, 'r', encoding='utf-8').read()
    r = generator.mergeDict(text, 1, minfreq, '正在合并360万中文词库')
    print('成功合并360万中文词库 %s 个汉字， %s 个词组。' % r)

    # 合并 rime 自带的八股文，权值 60
    essay_dict = os.path.join(submodules_dir, 'rime-essay', 'essay.txt')
    text = open(essay_dict, 'r', encoding='utf-8').read()
    r = generator.mergeDict(text, 60, 0, '正在合并八股文')
    print('成功合并八股文 %s 个汉字， %s 个词组。' % r)

    # 合并袖珍简化字拼音的词库，权值 60
    pinyin_simp_dict = os.path.join(
        submodules_dir, 'rime-pinyin-simp', 'pinyin_simp.dict.yaml')
    text = open(pinyin_simp_dict, 'r', encoding='utf-8').read(
    ).replace('罗嗦', '啰嗦')
    r = generator.mergeDict(text, 60, 0, '正在合并袖珍简化字拼音的词库')
    print('成功合并袖珍简化字拼音 %s 个汉字， %s 个词组。' % r)

    # 合并转换符号词汇
    symbols_dict = os.path.join(current_dir, '..', 'dicts', 'symbols.txt')
    text = open(symbols_dict, 'r', encoding='utf-8').read()
    r = generator.mergeDict(text, 10000, 0, '正在合并符号词汇 (%s/%s)')
...
sort: by_weight
...
""" % parse_dict_name + generator.getParseDictText('正在取得每个词组的拼音')

    word_dict_path = os.path.join(
        current_dir, '..', 'dicts', word_dict_name + '.dict.yaml')
    parse_dict_path = os.path.join(
        current_dir, '..', 'dicts', parse_dict_name + '.dict.yaml')

    open(word_dict_path, 'w').write(word_dict_text)
    open(parse_dict_path, 'w').write(parse_dict_text)
    return 0
